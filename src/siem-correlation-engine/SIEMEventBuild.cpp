...
bool CSIEMEventBuild::ZMQEventBuild(::SIEM::SIEMEvent& event, ::SIEM::SIEMPbMessage& pbMsg)
{
    ...
    {
        event.enEventType = (SIEM_EVENT_TYPE)pPbEvent->event_type_enum();
    }
    else
    {
    ...
    {
        event.enEventProtoType = (SIEM_PROTOCOL_TYPE)pPbEvent->protocol_type_enum();
    }
...
bool CSIEMEventBuild::ThriftEventBuild(::SIEM::SIEMEvent& event, const ::SIEM::thrift::SIEMThriftEvent& tEvent)
{
    Poco::Logger & logger = Poco::Util::Application::instance().logger();
    ...
    {
        event.enEventType = (SIEM_EVENT_TYPE)tEvent.event_type_enum;
    }
    else
    {
        logger.error("event type is null");
    ...
    {
        event.enEventProtoType = (SIEM_PROTOCOL_TYPE)tEvent.protocol_type_enum;
    }
